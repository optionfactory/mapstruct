...
public class TypeAnnotationSelector implements MethodSelector {
        ...
        final List<Candidate<SelectedMethod<T>>> compatibleMethods = methods.stream()
                .filter(method -> {
                    ...
                    for (int i = 0; i < sourceTypesAnnotations.size(); i++) {
                        ...
                        Set<DeclaredType> methodParameterTypeAnnotations = parameters.get(i).getType().getTypeMirror().getAnnotationMirrors().stream().map(m -> m.getAnnotationType()).collect(Collectors.toSet());
                    ...
                    Set<DeclaredType> returnTypeAnnotations = returnType.getTypeMirror().getAnnotationMirrors().stream().map(m -> m.getAnnotationType()).collect(Collectors.toSet());
                    return targetTypeAnnotations.containsAll(returnTypeAnnotations);
                })
                .map(m -> new Candidate<>(m, specificity(m, sourceTypes.size())))
                .collect(Collectors.toList());
        final long maxScore = compatibleMethods.stream().mapToLong(c -> c.score).max().orElse(0l);
        return compatibleMethods.stream().filter(c -> c.score == maxScore).map(c -> c.candidate).collect(Collectors.toList());
    }
    ...
    private static class Candidate<T> {
        ...
        public Candidate(T candidate, long score) {
    ...
    private long specificity(SelectedMethod<?> method, int sourceTypesCount) {
        ...
        final long matchingParameterTypeAnnotations = parameters.subList(0, sourceTypesCount).stream()
                .mapToLong(p -> p.getType().getTypeMirror().getAnnotationMirrors().stream().map(m -> m.getAnnotationType()).distinct().count())
                .sum();
        final Type returnType = method.getMethod().getResultType();
        final long matchingReturnTypeAnnotations = returnType.getTypeMirror().getAnnotationMirrors().stream().map(m -> m.getAnnotationType()).distinct().count();
